...
// Интерфейс соответствует данным от FastAPI (snake_case)
interface NewsItem {
    id: string;
    title: string;
    description: string;
        ...
        const fetchNews = async () => {
            ...
            try {
                ...
                if (err instanceof Error) {
                    ...
                    setError("Не удалось загрузить новости. Произошла неизвестная ошибка.");
                }
                console.error(err);
            } finally {
                setLoading(false);
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {news.length > 0 ? (
                                ...
                                <p className="col-span-full text-center text-gray-500">Новости по вашему запросу не найдены.</p>
                            )}
                        </div>
                    )}
                </section>
